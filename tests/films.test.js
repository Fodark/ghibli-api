...
test('fetch a single film by UUID', async () => {
    const resp = await getFilms('2baf70d1-42bb-4437-b551-e5fed5a87abe');
    expect(resp.ok).toBe(true);
...
test('fetch a non-existing film', async () => {
    const resp = await getFilms('b8c69609-cab8-486f-90ae-fc5b443decf7');
    expect(resp.ok).toBe(false);
});

test('fetch every miyazaki film', async () => {
    const resp = await getFilms({'director': 'miyazaki'});
    expect(resp.ok).toBe(true);
...
test('search a single film by name', async () => {
    const resp = await getFilms({'title': 'marnie'});
    expect(resp.ok).toBe(true);
    expect(resp.data).toBeInstanceOf(Array);
